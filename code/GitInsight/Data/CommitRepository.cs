...
public class CommitRepository : ICommitRepository
{
    ...
    public int Create(CommitCreateDTO commit)
    {
        ...
        var search = _context.Commits.Where(x => x.HashCode.Equals(commit.HashCode)).FirstOrDefault();

        /* If not add it to the database */
        if(search is null) 
        {
            var com = new DBCommit { Date = commit.Date, HashCode = commit.HashCode, Author = commit.Author,  BelongsTo = commit.BelongsTo };
    ...
    public CommitDTO Find(int commitId)
    {
        var com = from c in _context.Commits
                   where c.Id == commitId
                   select new CommitDTO(c.Id, c.HashCode, c.Date, c.Author, c.BelongsTo);
